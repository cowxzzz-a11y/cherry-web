...
export async function getAvailableProviders(): Promise<Provider[]> {
  try {
    ...
    // First try to get from admin_config.json
    try {
      const adminConfig = configStore.getState()
      if (
        adminConfig &&
        adminConfig.providers &&
        Array.isArray(adminConfig.providers) &&
        adminConfig.providers.length > 0
      ) {
        const providers = adminConfig.providers.map((p: any) => ({
          id: p.provider || p.id,
          type: p.provider || 'openai',
          name: p.provider || p.id,
          apiHost: p.apiHost || p.baseURL,
          apiKey: p.apiKey || p.api_key,
          enabled: true,
          models: (p.models || []).map((m: string) => ({
            id: m,
            name: m,
            provider: p.provider
          }))
        ...
        // Cache the results
        CacheService.set(PROVIDERS_CACHE_KEY, providers, PROVIDERS_CACHE_TTL)

        logger.info('Providers loaded from admin_config.json', {
          count: providers.length
        })
        return providers
      }
    } catch (configError) {
      logger.warn('Failed to load providers from admin_config.json', { configError })
    }

    // Fallback: try to get from Redux
