...
import type { ModelValidationError } from '../utils/api-utils';
import { validateModelId } from '../utils/api-utils';
...
export class ChatCompletionService {
  ...
  async prepareRequest(request: ChatCompletionCreateParams, stream: boolean): Promise<PrepareRequestResult> {
    ...
    if (!providerContext.ok) {
      return {
        status: 'model_error',
        error: (providerContext as any).error
      }
    }
    ...
    return {
      ...
      modelId,
      client,
      providerRequest: providerRequest as ChatCompletionCreateParams
    }
  }
  ...
  async processCompletion(request: ChatCompletionCreateParams): Promise<{
    ...
    try {
      ...
      const preparation = (await this.prepareRequest(request, false)) as any;
  ...
  async processStreamingCompletion(request: ChatCompletionCreateParams): Promise<{
    ...
    try {
      ...
      const preparation = (await this.prepareRequest(request, true)) as any;
      if (preparation.status === 'validation_error') {
        throw new ChatCompletionValidationError(preparation.errors)
      }
