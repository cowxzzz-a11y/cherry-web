...
export const mime2type = (mimeStr: string): FileType => {
  const mimeType = (mimeStr || '').toLowerCase().split(';')[0].trim()
  ...
  const documentMimeTypes = new Set(['application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument'])
  if (documentMimeTypes.has(mimeType) || mimeType.startsWith('application/vnd')) {
    return FILE_TYPE.DOCUMENT
  }

  const extFromMime = mimeType.includes('/') ? mimeType.split('/')[1] : ''
  if (extFromMime) {
    if (textExts.includes(extFromMime)) return FILE_TYPE.TEXT
    if (imageExts.includes(extFromMime)) return FILE_TYPE.IMAGE
    if (documentExts.includes(extFromMime)) return FILE_TYPE.DOCUMENT
    if (audioExts.includes(extFromMime)) return FILE_TYPE.AUDIO
    if (videoExts.includes(extFromMime)) return FILE_TYPE.VIDEO
  }
