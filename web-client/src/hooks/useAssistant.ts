...
import { setDefaultModel, setQuickModel, setTranslateModel, setDefaultEmbeddingModel, setDefaultRerankModel } from '@renderer/store/llm'
...
export function useDefaultModel() {
  const { defaultModel, quickModel, translateModel, defaultEmbeddingModel, defaultRerankModel } = useAppSelector((state) => state.llm)
  const dispatch = useAppDispatch()
  ...
  return {
    ...
    setDefaultEmbeddingModel: (model: Model | null) => dispatch(setDefaultEmbeddingModel({ model })),
    setDefaultRerankModel: (model: Model | null) => dispatch(setDefaultRerankModel({ model }))
  }
}
