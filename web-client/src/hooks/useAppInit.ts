  ...
  useEffect(() => {
    ...
    const syncFromServer = async () => {
      ...
      try {
        ...
        if (adminConfig?.providers && Array.isArray(adminConfig.providers)) {
          for (const ap of adminConfig.providers) {
            const id = ap.id || ap.provider
            if (id && (ap.apiKey || ap.api_key)) {
              dispatch(updateProvider({
                id,
                apiKey: ap.apiKey || ap.api_key || '',
                apiHost: ap.apiHost || ap.baseURL || '',
                enabled: true
              }))
            }
          }
