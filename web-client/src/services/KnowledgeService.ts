...
import { hasAPIVersion, routeToEndpoint, withoutTrailingSlash } from '@renderer/utils'
...
export const getKnowledgeBaseParams = (base: KnowledgeBase): KnowledgeBaseParams => {
  ...
  // Web mode: read provider config from Redux store (configured in Settings UI)
  // instead of using ModernAiProvider/AiProvider which have electron dependencies
  if (!isRealElectron) {
    ...
    // Apply provider-specific URL transformations
    ...
    } else if (baseURL && !hasAPIVersion(baseURL)) {
      // For standard OpenAI-compatible providers (e.g. SiliconFlow), ensure /v1 path is present.
      // The default apiHost in provider config may omit the version path (e.g. 'https://api.siliconflow.cn'),
      // but the OpenAI SDK needs the full base URL including /v1 to construct correct API endpoints.
      baseURL = withoutTrailingSlash(baseURL) + '/v1'
    }

    let rerankBaseURL = ''
    if (base.rerankModel?.provider) {
      const { baseURL: rHost } = routeToEndpoint(rerankProvider.apiHost)
      rerankBaseURL = rHost
      // Also ensure rerank URL has version path for standard providers
      if (
        rerankBaseURL &&
        !hasAPIVersion(rerankBaseURL) &&
        !isGeminiProvider(rerankProvider) &&
        !isAzureOpenAIProvider(rerankProvider) &&
        rerankProvider.id !== SystemProviderIds.ollama
      ) {
        rerankBaseURL = withoutTrailingSlash(rerankBaseURL) + '/v1'
      }
    }
  ...
  return {
    ...
    embedApiClient: {
      model: base.model.id,
      provider: base.model.provider,
      apiKey: aiProvider.getApiKey() || (isRealElectron ? 'secret' : ''),
      baseURL
    },
    chunkSize,
    chunkOverlap: base.chunkOverlap,
    rerankApiClient: {
      ...
      apiKey: rerankAiProvider.getApiKey() || (isRealElectron ? 'secret' : ''),
      baseURL: rerankHost
    },
    documentCount: base.documentCount ?? DEFAULT_KNOWLEDGE_DOCUMENT_COUNT,
...
export const getKnowledgeSourceUrl = async (item: KnowledgeSearchResult & { file: FileMetadata | null }) => {
  ...
  // For local file paths, extract just the filename for cleaner display
