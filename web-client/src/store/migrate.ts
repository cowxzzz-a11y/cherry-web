...
const migrateConfig = {
  ...
  '200': (state: RootState) => {
    try {
      if (!state.llm?.providers || state.llm.providers.length === 0) {
        state.llm.providers = SYSTEM_PROVIDERS.map((p) => ({
          ...p,
          models: Array.isArray((p as any).models) ? (p as any).models.map((m: any) => ({ ...m })) : []
        })) as any
      } else {
        fixMissingProvider(state)
      }
