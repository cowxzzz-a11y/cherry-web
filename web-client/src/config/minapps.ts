  ...
  try {
    ...
    try {
      if (!window.api?.file?.read) {
        const stored = localStorage.getItem('custom-minapps')
        if (stored) {
          content = stored
        } else {
          content = '[]'
          localStorage.setItem('custom-minapps', content)
        }
      } else {
      ...
      content = '[]'
      if (window.api?.file?.writeWithId) {
...
loadCustomMiniApp().then((apps) => {
  allMinApps = [...ORIGIN_DEFAULT_MIN_APPS, ...apps]
})
